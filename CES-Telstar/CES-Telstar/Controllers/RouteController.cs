...
namespace CES_Telstar.Controllers
{
    ...
    public class RouteController : ApiController
    {
        ...
        public RouteController(ISecurityService securityService, IRouteService routeService, ILocationService locationService)
        {
        ...
        {
            if (!_securityService.IsAuthenticated(apiKey)) return Unauthorized();
            ...
            var cheapestInternalRoute = _routeService.FindCheapest(startLocation, endLocation, true);
            var fastestInternalRoute = _routeService.FindFastest(startLocation, endLocation, true);

            var viewRoutes = new[]
            {
                RouteMapper.ViewRouteFromRoute(cheapestInternalRoute, startLocationName, endLocationName, packageType, recommended),
                RouteMapper.ViewRouteFromRoute(fastestInternalRoute, startLocationName, endLocationName, packageType, recommended)
            };
